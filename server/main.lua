...
local ver = "7.2"
local licekey = "OK"
local dev = "CODERC-SLO 100% HOMO"
local resv = "PHONE CODERC"
local licensedy = ""
local scm = ''
local ip
local license = "127.0.0.1:30120"
--------------------------------------------------------------------CEKLICENSE----------------------------------------------------------------------------
        ...
        if Ply.PlayerData.money.bank >= amount then
            ...
            CashoutCore.Functions.ExecuteSql(false, "SELECT * FROM `society_invoice` WHERE `society` = '"..socie.."'", function(result2)
                if result2[1] ~= nil then
                    ...
                    TriggerClientEvent('CashoutCore:Notify', src, "You payd invoice ") 
                end             
                end
                cb(true, result2)
            end)          
    ...
    CashoutCore.Functions.ExecuteSql(false, "SELECT * FROM `society_invoice` WHERE `society` = '"..socie.."'", function(result2)
        ...
        CashoutCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `citizenid` = '"..invoiceId.."'", function(result)
            if result[1] ~= nil then
               ...
               -- QBCore.Functions.ExecuteSql(true, "UPDATE `players` SET `money` = '"..json.encode(moneyInfo).."' WHERE `citizenid` = '"..invoiceId.."'")
                Ply.Functions.RemoveMoney('bank', amount, "paid-invoice")
                CashoutCore.Functions.ExecuteSql(true, "DELETE FROM `phone_invoices` WHERE `amount` = '"..amount.."'")
        ...
        TriggerClientEvent('qb-phone_new:client:DeclineInvoice', src)
