...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
        if (amountData >= amount) {
            $.post('http://coderc-phone/TransferMoney', JSON.stringify({
                iban: iban,
                amount: amount
            }), function(data){
                if (data.CanTransfer) {
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://coderc-phone/PayInvoice', JSON.stringify({
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('http://coderc-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        invoiceId: InvoiceData.invoiceid,
    }));
...
QB.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
        ...
        $.each(invoices, function(i, invoice){
            var Elem = '<div class="bank-app-invoice" id="invoiceid-'+i+'"> <div class="bank-app-invoice-title">'+invoice.society+' <span style="font-size: 1vh; color: gray;">(Sender: '+invoice.name+')</span></div> <div class="bank-app-invoice-amount">&#36; '+invoice.amount+',-</div> <div class="bank-app-invoice-buttons"> <i class="fas fa-check-circle pay-invoice"></i>  </div> </div>';
