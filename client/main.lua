...
RegisterNetEvent('ESX:Client:OnJobUpdate')
AddEventHandler('ESX:Client:OnJobUpdate', function(JobInfo)
            ...
            if not PhoneData.isOpen then
               -- local IsHandcuffed = exports['gov-police']:IsHandcuffed()
                if not IsHandcuffed then
                    ...
                    ESX.Functions.Notify("Action currently not possible..", "error")
                end
            end
        end
        Citizen.Wait(3)
...
function test()
    ...
    ESX.Functions.TriggerCallback('cash-telephone:server:GetPhoneData', function(pData)
...
function LoadPhone()
    ...
    isLoggedIn = true
    ESX.Functions.TriggerCallback('cash-telephone:server:GetPhoneData', function(pData)
        PlayerJob = ESX.Functions.GetPlayerData().job
        PhoneData.PlayerData = ESX.Functions.GetPlayerData()
...
RegisterNUICallback('HasPhone', function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:HasPhone', function(HasPhone)
        cb(HasPhone)
    end)
end)
local cek = false
function OpenPhone()
    ...
    ESX.Functions.TriggerCallback('cash-telephone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            ESX.Functions.TriggerCallback('cash-telephone:server:GetGarageVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
            
        else
            ESX.Functions.Notify("You don't have a phone", "error")
        end
        ---------------------------------fattupdate
        ESX.Functions.TriggerCallback('cash-telephone:server:GetPhoneData', function(pData)
            PlayerJob = ESX.Functions.GetPlayerData().job
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    ESX.Functions.TriggerCallback('cash-telephone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    ESX.Functions.TriggerCallback('cash-telephone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
AddEventHandler('cash-telephone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            Wait(100)
            ESX.Functions.TriggerCallback('cash-telephone:server:GetContactPictures', function(Chats)
                SendNUIMessage({
                    action = "UpdateChat",
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    ESX.Functions.TriggerCallback('cash-telephone:server:PayInvoice', function(CanPay, Invoices)
...
AddEventHandler('cash-telephone:client:DeclineInvoice', function()
    ...
    ESX.Functions.TriggerCallback('cash-telephone:server:DeclineInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('CallContact', function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:GetCallState', function(CanCall, IsOnline)
        local status = { 
            CanCall = CanCall, 
            IsOnline = IsOnline,
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:FetchResult', function(result)
        --print('test')
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('FetchVehicleResults', function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then 
            for k, v in pairs(result) do
                ESX.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    local vehicle = ESX.Functions.GetClosestVehicle()
    local plate = GetVehicleNumberPlateText(vehicle)
    local model = GetEntityModel(vehicle)

    ESX.Functions.TriggerCallback('cash-telephone:server:ScanPlate', function(result)
        ESX.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
            local vehicleInfo = ESX.Shared.Vehicles[ESX.Shared.VehicleModels[model]["model"]] ~= nil and ESX.Shared.Vehicles[ESX.Shared.VehicleModels[model]["model"]] or {["brand"] = "Brand..", ["name"] = ""}
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    ESX.Functions.Notify('GPS Locatie ingesteld: '..data.alert.title)
...
function GetClosestPlayer()
    local closestPlayers = ESX.Functions.GetPlayersFromCoords()
...
AddEventHandler('cash-telephone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        ESX.Functions.Notify("No one around!", "error")
    end
end)
...
RegisterNUICallback('GetCryptoData', function(data, cb)
    ESX.Functions.TriggerCallback('qb-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    ESX.Functions.TriggerCallback('qb-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    ESX.Functions.TriggerCallback('qb-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    ESX.Functions.TriggerCallback('qb-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    ESX.Functions.TriggerCallback('qb-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)
...
RegisterNUICallback('GetRaces', function(data, cb)
    ESX.Functions.TriggerCallback('qb-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    ESX.Functions.TriggerCallback('qb-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('HasCreatedRace', function(data, cb)
    ESX.Functions.TriggerCallback('qb-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)
...
RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    ESX.Functions.TriggerCallback('qb-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        local data = {
...
RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    ESX.Functions.TriggerCallback('qb-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    ESX.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        local ped = GetPlayerPed(-1)
        ...
        if dist <= 115.0 then
            ...
            ESX.Functions.Notify('You are too far from the race. Your navigation is set to the race.', 'error', 5000)
...
RegisterNUICallback('CanRaceSetup', function(data, cb)
    ESX.Functions.TriggerCallback('qb-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)
...
RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    ESX.Functions.Notify('GPS is set!', 'success')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)

RegisterNUICallback('GetCurrentpolices', function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:GetCurrentpolices', function(lawyers)
        cb(lawyers)
    end)
end)

RegisterNUICallback("GetCurrentDrivers", function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:GetCurrentDrivers', function(drivers)
        cb(drivers)
    end)
end)

RegisterNUICallback("GetCurrentMecano", function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:GetCurrentMecano', function(drivers)
        cb(drivers)
    end)
end)

RegisterNUICallback("GetCurrentDoctor", function(data, cb)
    ESX.Functions.TriggerCallback('cash-telephone:server:GetCurrentDoctor', function(drivers)
        cb(drivers)
    end)
end)
...
AddEventHandler('cash-telephone:client:spawnVehicle', function(data)
    ...
    if Config.RentelVehicles[data.model]['stored'] then
     ...
     ESX.Functions.SpawnVehicle(data.model, function(veh)
...
function RentVehicle(model)
    ...
    if Config.RentelVehicles[model] and not Config.RentelVehicles[model]['stored'] and location and Config.RentelVehicles[model].price <= ESX.Functions.GetPlayerData()['money']['bank'] then
